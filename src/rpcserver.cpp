...
void DynamicCheckpointsRefreshThread(void);
...
{ //  category              name                      actor (function)         okSafeMode threadSafe reqWallet
  //  --------------------- ------------------------  -----------------------  ---------- ---------- ---------
    ...
    /* Dynamic checkpoints */
    { "util",               "getonlinecheckpoint",    &getonlinecheckpoint,    true,      true,       false },
...
void StartRPCThreads()
{
         ...
         (mapArgs["-rpcuser"] == mapArgs["-rpcpassword"])) && Params().RequireRPCPassword())
    {
        ...
        uiInterface.ThreadSafeMessageBox(strprintf(
            _("To use octacoind, or the -server option to octacoin-qt, you must set an rpcpassword in the configuration file:\n"
              "%s\n"
    ...
    /* Dynamic checkpoints refresh */
    rpc_worker_group->create_thread(DynamicCheckpointsRefreshThread);
...
std::string HelpExampleCli(string methodname, string args){
    return "> octacoin-cli " + methodname + " " + args + "\n";
}

std::string HelpExampleRpc(string methodname, string args){
    return "> curl --user myusername --data-binary '{\"jsonrpc\": \"1.0\", \"id\":\"curltest\", "
        "\"method\": \"" + methodname + "\", \"params\": [" + args + "] }' -H 'content-type: text/plain;' http://127.0.0.1:4246/\n";
}

const CRPCTable tableRPC;

/**
 * A thread which refreshes the dynamic checkpoints data.
 */
void DynamicCheckpointsRefreshThread(void) {
    while (fRPCRunning) { /* Yup, cached reads. Exiting this thread isn't performance-sensitive */
        for (int i = 0; i < 60; i++) {
            boost::this_thread::sleep_for(boost::chrono::milliseconds(1000)); /* Check fRPCRunning each second */
            if (fRPCRunning)
                return;
        }
